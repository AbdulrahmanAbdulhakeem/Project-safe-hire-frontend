...
export default function CompaniesTable({ companies }: { companies: any[] }) {
  ...
  const handleDelete = async (userId: string | undefined, name: string) => {
  if (!userId) {
    toast.error('Missing user id — refresh and try again');
    return;
  }
            ...
            {companies.length === 0 ? (
              ...
              companies.map((company) => (
                <TableRow key={company.id}>
                  ...
                  <TableCell>
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${company.status === "APPROVED" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                    >
                      {company.status}
                    </span>
                  </TableCell>
                  ...
                  <TableCell className="text-right space-x-2">
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => handleDelete(company.userId, company.name)}
                      disabled={
                        !company.userId || deletingId === company.userId
                      }
                    >
